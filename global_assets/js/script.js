const process = () => {
    ...
    const data = {
        "Policy #" : ['','CAS000153/1800', 'CAS000153/1800', 'CAS000153/1800', 'CAS000153/1900', 'CAS000153/1900', 'CAS000153/1900'],
        "Claim #" : ['','ADIH18070004','ADIH18080008','ADIH18100016', 'ADIH19080007','ADIH19120026', 'ADIH20020006'],
        "Status": ['','Closed','Open','Closed','Closed','Closed','Open'],
        "Loss" : ['Date','06/10/2018','07/27/2018','08/23/2018','08/01/2019','12/26/2019', '11/19/2019'],
        "Loss State" : ['','TX','TX','TX','TX','TX','TX'],
        "Cause of" : ['Loss','Slip and Fall', 'Property Damage - NOC', 'Slip and Fall', 'All Other Property - NOC','Fall (Non-Slip Trip)', 'Collapse'],
        "Total" : ['Reserve','$ 0.00', '$ 12,490.00','$ 0.00','$ 0.00','$ 0.00','$ 21,655.00'],
        "Total P": ['', '$' ,'$' ,'$' ,'$' ,'$' ,'$' ,],
        "aid" : ['', ' 21,655.00',' 990.00',' 3,201.52',' 6,017.90', ' 1,642.59', ' 456.50'],
    ...
    const data2 = {
        "Policy #" : ['CAS000153/1800','CAS000153/1900'],
        "Effective" : ['05/15/2018', '05/15/2019'],
        "Expiration" : ['05/15/2019', '05/15/2020'],
        "# of Claims" : [3, 3] 
    }
...
const showHover = e => {
    ...
    hover.innerHTML = `<button class="fa fa-arrow-left-long"></button>`
...
const editTable = e => {
    ...
    tools.forEach(tool => {
        tool.classList.toggle('show-col');
    })
    const toolRows = document.querySelectorAll('.table .tool-row')
    toolRows.forEach(tool => tool.classList.toggle('show-row'))
    document.getElementById('editTableButton').classList.toggle('selected-button')
    
    let headings = document.querySelectorAll('.table .row .col-heading');

    headings.forEach(heading => heading.classList.toggle('hover-class') )
...
const drawTable = data => {
    ...
    // table.style.width = (lrResult.offsetWidth > table.offsetWidth ? lrResult.offsetWidth : table.offsetWidth) + '! important';
    ...
    Object.keys(data).forEach(key => {
        ...
        input.title = key;
        input.value = key;
        mergeBtn.addEventListener('click', mergeLeft);
        mergeBtn.classList.add('fa','fa-arrow-left-long')
    ...
    for(let i = 0; i < Object.values(data)[0].length; i++){
        ...
        // edit.classList.add('fa','fa-pen');
        const merge = document.createElement('BUTTON')
        merge.addEventListener('click', mergeUp)
    ...
    //     const merge = document.createElement('BUTTON')
    //     merge.classList.add('fa','fa-arrow-left-long')
    //     merge.addEventListener('click', mergeLeft)
    //     // col.appendChild(edit)
    //     col.appendChild(merge)
    //     row.appendChild(col)
    // }
